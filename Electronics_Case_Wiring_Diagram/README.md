...
I am tired for now and it looks like I will have to update my [wiring harness diagram](./Wiring_Harness_Diagram/Voron_2.4_Tool_Head_PCB__Wiring_Harness.pdf) to reflect the changes to the Klicky probe connection and the Filament Sensor (for ERCF connection) to the Octopus Pro board. I apperently need to swap those two locations. I also need to change the LED hook up on the wiring harness diagram.  I have switched from 24VDC LED lights to only using Neopixel lights (which are 5VDC, GND and a data line).
